...
router.post("/create", decodeIDToken, async (req, res) => {
  try {
    ...
    let { title, body, description, topicsTagged, courseOutcome } = req.body;
    if (!title || !body || !description) {
      ...
      if (lodash.isEmpty(courseOutcome)) throw "Course Outcome is required";
...
router.put("/:id/unregister", decodeIDToken, async (req, res) => {
  try {
    ...
    if (!email) {
      ...
      if (!data) throw "Error while unregistering the course, please try again";
      if (!data) throw "Error in creating the new course";
      return res.status(200).json({ data: data });
    }
  } catch (err) {
...
router.delete("/:id", decodeIDToken, async (req, res) => {
  try {
    ...
    if (!email) {
      ...
      if (!data) throw "Error while deleting the course, please try again";
      return res.status(200).json({ data: data });
    }
  } catch (err) {
    console.log("err>>>>>>>>>>>>>>>>>>>>>>", err);
